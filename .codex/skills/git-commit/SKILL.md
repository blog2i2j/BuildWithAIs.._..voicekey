...
| `style`    | 代码格式（不影响运行）  | style(lint): fix eslint warnings            |
| `refactor` | 重构（无功能变化）      | refactor(user): extract validation logic    |
| `perf`     | 性能优化                | perf(query): optimize database queries      |
| `test`     | 测试相关                | test(user): add unit tests for user service |
| `build`    | 构建系统/依赖           | build(deps): upgrade vue to 3.4             |
| `ci`       | CI/CD 配置              | ci(actions): add deploy workflow            |
| `chore`    | 杂项（不修改 src/test） | chore(scripts): update build script         |
| `revert`   | 回滚提交                | revert: revert commit abc123                |

### Scope 范围说明

Scope 表示本次修改影响的范围，根据项目结构定义：
...
# ❌ 错误示例
feat(auth): Added new feature.     # 不要用过去式，不要加句号
fix: bug                           # 描述太模糊
FEAT(AUTH): ADD LOGIN              # 不要全大写
feat(auth): 添加登录功能            # 建议使用英文
```
...
Implement Stripe payment gateway for processing credit card payments.
...
This replaces the legacy PayPal integration.
```
...
Add Google OAuth2 authentication as an alternative to email/password login.

- Configure Google OAuth2 credentials
- Add GoogleAuthService with token validation
- Create /auth/google callback endpoint
- Store user profile from Google

The new login flow improves user onboarding experience.
